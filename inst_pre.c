...
*/
       ...
       */
            if (gtype_size[0] == MAX_TYPE) {
                printf(
                    "\ntoo many consts in type %s! increase MAX_TYPE (currently "
                    "%d)\n\n",
    ...
    for (o = gloaded_ops; o; o = o->next) {
        ...
        for (ff = o->params; ff; ff = ff->next) {
            if ((type_num = position_in_types_table(ff->item->next->item)) == -1) {
                printf(
                    "\nwarning: parameter %s of op %s has unknown or empty type %s. "
                    "skipping op",
            ...
            for (i = 0; i < tmp->num_vars; i++) {
                if (tmp->var_names[i] == ff->item->item ||
                    ...
                    printf(
                        "\nwarning: operator %s parameter %s overwrites previous "
                        "declaration\n\n",
...

{
    ...
    for (t = n->atom->next; t; t = t->next) {
         ...
         */
            ...
            if (f->predicate != -1 &&
                ...
                printf(
                    "\ntype of var %s does not match type of arg %d of predicate "
                    "%s\n\n",
            ...
            if (f->predicate != -1 &&
                ...
                printf(
                    "\ntype mismatch: constant %s as arg %d of %s. check input "
                    "files\n\n",
    ...
    if (f->predicate == -1) {
        ...
        if (m != garity[f->predicate]) {
            printf(
                "\npredicate %s is declared to have %d (not %d) arguments. check "
                "input files\n\n",
...

{
    ...
    switch (p->connective) {
        ...
        case EX:
            for (i = 0; i < num_vars; i++) {
                if (lvar_names[i] == p->atom->item ||
                    ...
                    printf(
                        "\nwarning: var quantification of %s overwrites previous "
                        "declaration\n\n",
                        p->atom->item);
                }
            }
            if ((t = position_in_types_table(p->atom->next->item)) == -1) {
                printf(
                    "\nwarning: quantified var %s has unknown or empty type %s. "
                    "simplifying.\n\n",
        ...
        case OR:
            if (!p->sons) {
                printf(
                    "\nwarning: empty con/disjunction in domain definition. "
                    "simplifying.\n\n");
...

{
    ...
    for (n = p; n && n->connective == ALL; n = n->sons) {
        if ((t = position_in_types_table(n->atom->next->item)) == -1) {
            printf(
                "\nwarning: effect parameter %s has unknown or empty type %s. "
                "skipping effect.\n\n",
        ...
        for (i = 0; i < num_vars + tmp->num_vars; i++) {
            if (lvar_names[i] == n->atom->item ||
                ...
                printf(
                    "\nwarning: effect parameter %s overwrites previous "
                    "declaration\n\n",
                    n->atom->item);
            }
        }
...

{
    ...
    if (gcmd_line.display_info == 105) {
        ...
        for (i = 0; i < gnum_predicates; i++) {
            printf("\n%3d --> %s: ", i, gpredicates[i]);
            printf(" is %s, %s", gis_added[i] ? "ADDED" : "NOT ADDED",
...

{
    ...
    for (i = 0; i < gnum_predicates; i++) {
        if (!gis_added[i] && !gis_deleted[i] && garity[i] == 1) {
            if (gnum_types == MAX_TYPES) {
                printf(
                    "\ntoo many (inferred) types! increase MAX_TYPES (currently "
                    "%d)\n\n",
                    MAX_TYPES);
                exit(1);
            }
   ...
   */
    ...
    for (i = 0; i < gnum_predicates; i++) {
        ginitial_predicate[i] =
            (Fact *)calloc(gnum_initial_predicate[i], sizeof(Fact));
        gnum_initial_predicate[i] = 0;
    }
    ginitial = NULL;
    gnum_initial = 0;
    ...
    for (i = 0; i < gnum_full_initial; i++) {
        ...
        for (j = 0; j < garity[p]; j++) {
            ginitial_predicate[p][gnum_initial_predicate[p]].args[j] =
                gfull_initial[i].args[j];
        }
        gnum_initial_predicate[p]++;
        if (gis_added[p] || gis_deleted[p]) {
            ...
            if (garity[p] == 1) {
                ...
                if (!gis_member[gfull_initial[i].args[0]]
                               [gpredicates_args_type[p][0]]) {
    ...
    switch ((*w)->connective) {
        ...
        case EX:
            ...
            for (j = 0; j < gtype_size[(*w)->var_type]; j++) {
                tmp = copy_Wff((*w)->son);
                expand_quantifiers_in_wff(&tmp, (*w)->var,
                                          gtype_consts[(*w)->var_type][j]);
                tmp->next = r;
                if (r) {
                    r->prev = tmp;
                }
...

{
    ...
    if (gnum_predicates == MAX_PREDICATES) {
        printf(
            "\ntoo many predicates in translation! increase MAX_PREDICATES "
            "(currently %d)\n\n",
            MAX_PREDICATES);
        exit(1);
    }
...

{
    ...
    geasy_operators =
        (NormOperator_pointer *)calloc(s, sizeof(NormOperator_pointer));
    ...
    for (i = 0; i < gnum_operators; i++) {
        ...
        switch (w->connective) {
            ...
            default:
                printf(
                    "\nwon't get here: non OR, AND, ATOM, TRUE, FALSE in dnf. debug "
                    "me\n\n");
                exit(1);
        }
    }
...

{
    ...
    if (first_call) {
        lset = (WffNode_pointer *)calloc(MAX_HITTING_SET_DEFAULT,
                                         sizeof(WffNode_pointer));
...

{
    ...
    switch ((*w)->connective) {
        ...
        case EX:
            printf(
                "\ntrying to put quantified formula into DNF! (ands down) debug "
                "me\n\n");
            exit(1);
            break;
        case AND:
