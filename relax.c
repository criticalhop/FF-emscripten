...
{
    make_state(&lcurrent_goals_, gnum_ft_conn);
    lcurrent_goals_.max_F = gnum_ft_conn;
}
...

{
    ...
    source_to_dest(&lcurrent_goals_, current_goals);
...

{
    ...
    for (i = 0; i < lcurrent_goals_.num_F; i++) {
        if (!gft_conn[lcurrent_goals_.F[i]].in_F) {
            return FALSE;
        }
    }

    for (i = 0; i < lcurrent_goals_.num_F; i++) {
        if (gft_conn[lcurrent_goals_.F[i]].level == INFINITY) {
            gft_conn[lcurrent_goals_.F[i]].level = time;
        }
    }
